(window.watchIn = window.watchIn || {}),
  (function () {
    function t_events__initEvent() {
      ...
      try {
        var allRec = document.getElementById("allrecords"),
          allRecCookie = allRec
            ? allRec.getAttribute("data-watchIn-cookie")
            : null;
        (allRec && "no" === allRecCookie) || watchIn.saveUTM();
      } catch (e) {}
      ...
      function linkClickCreateEvent(e) {
        ...
        if (e.target && targetEl) {
          var virtPage = targetEl.getAttribute("data-watchIn-event-name"),
          ...
          if (
            (watchIn.sendEventToStatistics(virtPage, virtTitle),
    ...
    (watchIn.sendEcommerceEvent = function (type, productsArr) {
      ...
      (watchIn.sendEventToStatistics = function (
        virtPage,
        virtTitle,
        referer,
        price
        ...
        if (
          ((currencyCode = allRec.getAttribute("data-watchIn-currency")
            ? allRec.getAttribute("data-watchIn-currency")
          ...
          if (
            ...
            (referer = {
              ecommerce: {
                purchase: {
                  actionField: {
                    id: window.watchInForm.orderIdForStat,
                    revenue: window.watchInForm.amountForStat,
                  },
                  products: window.watchInForm.arProductsForStat,
                },
              },
            }),
              window.watchInForm.watchInpayment &&
                window.watchInForm.watchInpayment.promocode &&
        ...
        if (
          ...
          if (isVirtPage)
            if (referer && referer.event) {
              try {
                if (
                  (window.watchIn.isLoadGAEcommerce ||
                    ((window.watchIn.isLoadGAEcommerce = !0),
                    ga("require", "ec")),
              ...
              if (
                ...
                if (referer && referer.event && price > 0)
                  if ("addToCart" === referer.event && referer.ecommerce.add) {
                    ...
                    virtPage.indexOf("watchIn/popup"),
                      window.fbq("track", "ViewContent", {
          ...
          try {
            if (isVirtPage) {
              ...
              if (referer && referer.event)
                ...
                eventName =
                  "t-purchase-" +
                  (tmp = (tmp = (tmp = virtPage.replace(
                    "/watchIn/",
                    ""
                  )).replace("watchIn/", "")).replace("/payment/", ""));
              else if (
                -1 !== virtPage.indexOf("watchIn/") &&
                -1 !== virtPage.indexOf("/submitted/")
              )
                eventName =
                  ...
                  (tmp = (tmp = (tmp = virtPage.replace(
                    "/watchIn/",
                    ""
                  )).replace("watchIn/", "")).replace("/submitted/", ""));
              else if (-1 !== virtPage.indexOf("watchIn/popup"))
                eventName =
                  "t-" +
                  (tmp = (tmp = virtPage.replace("/watchIn/", "")).replace(
                    "/",
                    "-"
                  ));
              else if (-1 !== virtPage.indexOf("watchIn/click"))
                eventName =
                  "t-" +
                  (tmp = (tmp = virtPage.replace("/watchIn/", "")).replace(
                    "/",
        ...
        "function" == typeof window.watchInstat &&
          (isVirtPage
            ? (virtPage.indexOf("payment") > 0 &&
                virtPage.indexOf("watchIn/form") > -1 &&
                (virtPage = virtPage.replace("watchIn/form", "watchIn/rec")),
              window.watchInstat("pageview", { page: virtPage }))
            : window.watchInstat("pageview", {
                page: "/watchIn/event/" + virtPage,
              }));
      }),
      (watchIn.saveUTM = function () {
        try {
          var watchInPageURL = window.location.href,
            watchInPageQuery = "",
            watchInPageUTM = "";
          if (
            -1 !== watchInPageURL.toLowerCase().indexOf("utm_") &&
            "string" ==
              typeof (watchInPageQuery = (watchInPageQuery = (watchInPageURL =
                watchInPageURL.toLowerCase()).split("?"))[1])
          ) {
            var arPair,
              arParams = watchInPageQuery.split("&");
            for (var i in arParams)
              "amp;" === (arPair = arParams[i].split("="))[0].substring(0, 4) &&
                (arPair[0] = arPair[0].substring(4)),
                "utm_" === arPair[0].substring(0, 4) &&
                  (watchInPageUTM = watchInPageUTM + arParams[i] + "|||");
            if (watchInPageUTM.length > 0) {
              var date = new Date();
              date.setDate(date.getDate() + 30),
                (document.cookie =
                  "watchInUTM=" +
                  encodeURIComponent(watchInPageUTM) +
                  "; path=/; expires=" +
                  date.toUTCString());
            }
          }
